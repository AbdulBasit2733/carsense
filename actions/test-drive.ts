...
export async function bookTestDrive({
  ...
  try {
    ...
    if (!userId) {
      return { success: false, error: "You must be logged in to book a test drive" };
    }
...
export async function getUserTestDrive() {
  try {
    const { userId } = await auth();
    if (!userId) {
      return { success: false, error: "You must be logged in to view your test drives" };
    }
    ...
    return { success: false, error: error?.message || "Failed to fetch test drives" };
  }
}
...
export async function cancelTestDrive(bookingId: any) {
  try {
    const { userId } = await auth();
    if (!userId) {
      return { success: false, error: "You must be logged in to cancel a test drive" };
    }
    ...
    return { success: false, error: error?.message || "Failed to cancel test drive" };
  }
}
